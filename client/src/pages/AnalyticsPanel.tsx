...
export default function AnalyticsPanel() {
  ...
  return <main className="panel-shell" id="analytics-report">
    ...
    <section className="panel-card leads-card"><div className="panel-card-heading"><div><span className="eyebrow">Pedidos de orçamento</span><span>{leads.data?.length ?? 0} registrados</span></div><button className="csv-button" onClick={exportLeadsCsv} disabled={!leads.data?.length}>Exportar CSV ↓</button></div><div className="leads-list">{(leads.data ?? []).map(lead => <article key={lead.id}><div className="lead-main"><div><strong>{lead.name}</strong><span>{lead.email}{lead.phone ? ` · ${lead.phone}` : ""}</span></div><label className="lead-status">Status<select value={lead.status} disabled={updateLeadStatus.isPending} onChange={event => updateLeadStatus.mutate({ id: lead.id, status: event.target.value as "pending" | "responded" | "completed" })}><option value="pending">Pendente</option><option value="responded">Respondido</option><option value="completed">Concluído</option></select></label></div><time>{new Date(lead.createdAt).toLocaleString("pt-BR")}</time><p>{lead.message}</p></article>)}</div></section>
  </main>;
}
