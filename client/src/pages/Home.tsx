...
import { ArrowDownRight, ArrowUp, ArrowUpRight, ChevronLeft, ChevronRight, ExternalLink, Moon, Search, Sun, X } from "lucide-react";
...
function CatalogImage({ src, alt, onError }: { src: string; alt: string; onError: () => void }) {
  const [loaded, setLoaded] = useState(false);
  return <div className="catalog-image">
    {!loaded && <span className="image-skeleton" aria-hidden="true" />}
    <img className={`protected-media${loaded ? " is-loaded" : ""}`} draggable={false} onContextMenu={event => event.preventDefault()} onError={onError} onLoad={() => setLoaded(true)} src={src} alt={alt} loading="lazy" decoding="async" />
    <span className="card-shade" /><span className="card-arrow"><ArrowUpRight size={18} /></span>
  </div>;
}
...
export default function Home() {
  ...
  const projectViewRef = useRef<HTMLDivElement | null>(null);
  ...
  const scrollProjectToTop = () => { const surface = projectViewRef.current; if (!surface) return; surface.scrollTo({ top: 0, left: 0, behavior: window.matchMedia("(prefers-reduced-motion: reduce)").matches ? "auto" : "smooth" }); surface.focus({ preventScroll: true }); };
  const changeProject = (direction: number) => { if (!selected || projectClosing || fullscreenOpen || filtered.length < 2) return; const index = filtered.findIndex(project => project.id === selected.id); if (index < 0) return; setKeyboardDirection(direction > 0 ? "right" : "left"); window.setTimeout(() => setKeyboardDirection(null), 720); openProject(filtered[(index + direction + filtered.length) % filtered.length]); };
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      frame = requestAnimationFrame(() => setShowBackToTop(window.scrollY > Math.max(420, window.innerHeight * 0.7)));
    };
    onScroll();
