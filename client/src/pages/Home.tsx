...
export default function Home() {
  ...
  const openProject = (project: PortfolioProject) => { setSelected(project); setMediaIndex(0); setFullscreenOpen(false); resetZoom(); };
  ...
  const downloadMedia = async (src: string, index: number) => { try { const response = await fetch(src); const blob = await response.blob(); const url = URL.createObjectURL(blob); const link = document.createElement("a"); link.href = url; link.download = `deva-braga-midia-${String(index + 1).padStart(2, "0")}`; link.click(); URL.revokeObjectURL(url); } catch { window.open(src, "_blank", "noopener,noreferrer"); } };
