...
export default function NotificationDeliveryHistory({ quotes }: { quotes: Quote[] }) {
  ...
  return <>
    ...
    <section className="panel-card notification-delivery-card" id="notification-delivery-history">
      ...
      {rows.length === 0 ? <p className="empty-panel-state">Nenhuma tentativa registrada ainda.</p> : <div className="delivery-history-list">{rows.slice(0, 80).map(item => { const quote = item.quoteRequestId ? quoteNames.get(item.quoteRequestId) : undefined; const failed = item.status === "failed"; return <article key={item.id} className={failed ? "delivery-failed" : "delivery-sent"}><div className="delivery-history-main"><span className="delivery-status-icon">{failed ? <AlertTriangle size={15} /> : <CheckCircle2 size={15} />}</span><div><strong>{item.channel === "email" ? "Gmail SMTP" : item.channel === "whatsapp" ? "Meta WhatsApp" : "Fallback interno"}</strong><span>{quote ? `Pedido #${quote.id} · ${quote.name}` : "Teste de provedor"} · {item.attemptType === "manual_retry" ? "reenvio manual" : item.attemptType === "test" ? "teste controlado" : "envio automático"}</span></div></div><div className="delivery-history-meta"><span>{failed ? `Falhou${item.errorCode ? ` · ${item.errorCode}` : ""}` : "Entregue"}</span><time>{new Date(item.createdAt).toLocaleString("pt-BR")}</time>{failed && item.quoteRequestId && <button className="retry-button" onClick={() => retry.mutate({ quoteRequestId: item.quoteRequestId as number })} disabled={retry.isPending}><RotateCw size={13} /> Reenviar falhas</button>}</div></article>; })}</div>}
    </section>
  </>;
}
