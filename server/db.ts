...
export async function upsertPortfolioProjectOverride(input: { projectKey: string; title?: string; description?: string; year?: string; thumbnail?: string; media?: string[]; mediaMetadata?: Record<string, { caption?: string; alt?: string; credit?: string }>; sourceUrl?: string; hidden?: boolean; allowDownloads?: boolean }) {
  const db = await getDb(); if (!db) throw new Error("DATABASE_URL is not configured");
  await db.insert(portfolioProjectOverrides).values({ projectKey: input.projectKey, title: input.title ?? null, description: input.description ?? null, year: input.year ?? null, thumbnail: input.thumbnail ?? null, media: input.media ? JSON.stringify(input.media) : null, mediaMetadata: input.mediaMetadata ? JSON.stringify(input.mediaMetadata) : null, sourceUrl: input.sourceUrl ?? null, hidden: input.hidden ? 1 : 0, allowDownloads: input.allowDownloads ? 1 : 0 }).onDuplicateKeyUpdate({ set: { title: input.title ?? null, description: input.description ?? null, year: input.year ?? null, thumbnail: input.thumbnail ?? null, media: input.media ? JSON.stringify(input.media) : null, mediaMetadata: input.mediaMetadata ? JSON.stringify(input.mediaMetadata) : null, sourceUrl: input.sourceUrl ?? null, hidden: input.hidden ? 1 : 0, allowDownloads: input.allowDownloads ? 1 : 0, updatedAt: new Date() } });
}
