...
export const appRouter = router({
  ...
  analytics: router({
    ...
    saveProjectOverride: adminProcedure.input(z.object({ projectKey: z.string().min(1).max(191), title: z.string().trim().min(1).max(500), description: z.string().max(10000), year: z.string().max(32), thumbnail: z.union([z.string().url().max(2000), z.literal("")]), sourceUrl: z.string().url().max(2000), hidden: z.boolean().default(false) })).mutation(async ({ input, ctx }) => { await upsertPortfolioProjectOverride(input); await createTemplatedNotification({ eventKey: "project_updated", variables: { projectKey: input.projectKey, title: input.title }, fallback: { title: "Projeto atualizado", message: `O projeto “${input.title}” foi atualizado no painel.`, severity: "success" } }); await recordAuditLog({ actor: ctx.user.email ?? ctx.user.name ?? ctx.user.openId, entityType: "project", entityKey: input.projectKey, action: "updated", details: JSON.stringify({ title: input.title, year: input.year }) }); return { ok: true as const }; }),
