...
export const appRouter = router({
  ...
  analytics: router({
    ...
    saveProjectOverride: adminProcedure.input(z.object({ projectKey: z.string().min(1).max(191), title: z.string().trim().min(1).max(500), description: z.string().max(10000), year: z.string().regex(/^\d{4}$/), thumbnail: z.union([z.string().url().max(2000), z.literal("")]), sourceUrl: z.string().url().max(2000), hidden: z.boolean().default(false), allowDownloads: z.boolean().default(false), license: z.string().trim().min(1).max(500).default("Todos os direitos reservados — uso mediante autorização."), mediaMetadata: z.record(z.string(), z.object({ caption: z.string().max(500).optional(), alt: z.string().max(500).optional(), credit: z.string().max(300).optional() })).optional() })).mutation(async ({ input, ctx }) => { await upsertPortfolioProjectOverride(input); await createTemplatedNotification({ eventKey: "project_updated", variables: { projectKey: input.projectKey, title: input.title }, fallback: { title: "Projeto atualizado", message: `O projeto “${input.title}” foi atualizado no painel.`, severity: "success" } }); await recordAuditLog({ actor: ctx.user.email ?? ctx.user.name ?? ctx.user.openId, entityType: "project", entityKey: input.projectKey, action: "updated", details: JSON.stringify({ title: input.title, year: input.year, allowDownloads: input.allowDownloads, license: input.license }) }); return { ok: true as const }; }),
